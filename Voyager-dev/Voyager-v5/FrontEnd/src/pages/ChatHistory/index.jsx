...
export default function ChatHistoryList() {
  ...
  useEffect(() => {
    const fetchThreads = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:3000/api/chat/threads", {
          headers: {
  ...
  const deleteThread = async (threadId) => {
    try {
      const token = await currentUser.getIdToken(true);
      await axios.delete(`http://localhost:3000/api/chat/threads/${threadId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const token = await currentUser.getIdToken(true);
        const response = await axios.get(`http://localhost:3000/api/files/uploadedFiles`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const deleteFile = async (fileName) => {
    try {
      const token = await currentUser.getIdToken(true);
      await axios.delete(`http://localhost:3000/api/files/deleteFirebasefile/${fileName}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
