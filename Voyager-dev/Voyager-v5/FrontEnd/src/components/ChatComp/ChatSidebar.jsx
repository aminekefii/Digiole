...
const Chatsidebar = () => {
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/files/downloads/${filename}`, { responseType: "blob" });
  ...
  const checkFileReady = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/files/file-ready/${filename}`);
  ...
  const handleExitChat = async () => {
    ...
    if (confirmExit) {
      try {
        await axios.delete('http://localhost:3000/api/chat/delete-assistant-files');
        navigate('/assistants');
      } catch (error) {
